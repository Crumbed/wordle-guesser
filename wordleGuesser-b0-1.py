# fix rows in probability function
...
def getProbability(posAns):
    ...
    for word in posAns:
        ...
        for line in simpleRows:
            if line[0:1] == wordFl:
                wordWeight += int(line[3: 9])
            if line[11: 12] == wordLl:
                wordWeight += int(line[14: 20])
            for x in range(len(word)):
                if len(word[x: x+2]) == 2:
                    if line[22: 24] == word[x: x+2]:
                        wordWeight += int(line[26: 32])
    ...
    index = 0
    for word in posAns:
        if f'{word} - {ansWeights[0]}' in ansAndWeights:
            return word

def filterPossibleGuesses(knownPos, correctLet, incorrectSub, notIn):
    global filtPosAns
...
def getNextGuess(ans, out):
    ...
    print(
        f'Known letter possitions: {knownLetPos}\nList of letters in word: {letInWord}\nList of letters NOT in word: {letNotInWord}')

    filtPosAns = filterPossibleGuesses(
        knownLetPos, letInWord, incorrectPos, letNotInWord)
    print(filtPosAns)
...
for i in range(6):
    print(f'Enter \"{inputedAns}\"')
    inputedOut = input(
        f'Enter the output of {inputedAns} (gray: 0, yellow: 1, green: #): ')
